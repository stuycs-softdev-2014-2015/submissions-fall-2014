...
def calcPercents(s):
    ...
    """
    percents = [ float(s.count(chr(i+97)))/(len(s)-s.count(" ")) * 100
                 for i in range(26)]
    return percents

def decode(s):
    distances = [dist(calcPercents(encode2(s,i)),englishPercents) for i in range(26)]
    rotation =  distances.index(min(distances))
    return encode2(s,rotation)
