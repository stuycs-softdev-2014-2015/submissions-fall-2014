...
#using a comprehension -- i tried to figure out a way to
#get rid of the loop and use zip() -- but i don't see it as of yet.
def dist2(a,b):
    sum = 0
...
import random
rotation = random.randrange(26)
encmessage = encode2(s, rotation)
...
#using some comprehensions code from class
def modifyEnglishPercents(fn):
    global englishPercents
    wordstring = open(fn).read().lower()
    letters = "abcdefghijklmnopqrstuvwxyz"
    wordstring = [x for x in wordstring if x in letters]
    length = len(wordstring)
    englishPercents = [(calcFrequency(chr(x),s) / float(length)) * 100 for x in range(97,123)]


modifyEnglishPercents('samplefile.txt')
