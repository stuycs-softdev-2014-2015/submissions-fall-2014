...
def findDecodeNum(encmessage):
    distList = [dist(calcPercents(encode2(encmessage,i)),englishPercents) for i in range(26)]
    decodeNum = distList.index(min(distList))
    return decodeNum
...
def letterFreq(file):
    f = open(file,'r')
    text = f.read().lower()
    return [100*text.count(x)/float(len(''.join(text.split()))) for x in string.lowercase]
...
print encode2(encmessage,findDecodeNum(encmessage))
print letterFreq('shakespeare.txt')
