...
def dist(a,b):
    ...
    """
    return math.sqrt(sum([pow(a[x] - b[x],2) for x in range(len(a))])) 
...
def decoding(message):
    distance = [dist(calcPercents(encode2(message,x)), englishPercents) for x in range(26)]
    return encode2(message, distance.index(min(distance)))

def calcOtherPercent():
    bloop = open('A_Portrait_of_the_Artist_as_a_Young_Man.txt','r')
    return calcPercents(bloop.read())

def decodingJoyce(message):
    distance = [dist(calcPercents(encode2(message,x)), calcOtherPercent()) for x in range(26)]
    return encode2(message, distance.index(min(distance)))
...
print encmessage
print decoding(encmessage) 
print decodingJoyce(encmessage)

# Your tasks
#1. Rewrite dist so that it uses a list comprehention -- DONE

#2. Finish writing calcPercents -- DONE
#
#3. For each of the 26 possible rotations of encmessage (the encoded message) 
#   see the distance between it and the englishPercents. The closest should
