...
def calcPercents(s):
    ...
    """
    l = [x.lower() for x in s if x.isalpha()]
    return [l.count(chr(x+97))/float(len(l))for x in range(26)]
...
def decode(s):
    d = [dist(calcPercents(encode2(s,x)),englishPercents) for x in range(26)]
