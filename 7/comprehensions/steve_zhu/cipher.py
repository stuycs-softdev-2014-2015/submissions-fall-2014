...
def calcPercents(s):
	...
	"""
	total = len(s)
	return[100 * float(s.count(chr(l)))/total for l in range(ord('a'), ord('z') + 1)]
...
def calcFreq(filename):
	...
	file = open(filename)
	text = file.read()
	text = [l for l in text if l in alphabet]
	count = len(text)
	return [100*float(text.count(l))/count for l in alphabet]

englishPercents = calcFreq('hamlet.txt')


def decode_distance(s):
	return min([dist(englishPercents, calcPercents(encode2(s, i))) for i in range(26)])
