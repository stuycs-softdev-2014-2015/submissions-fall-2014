...
def calcPercents(s):
    ...
    """
    return [float(s.count(chr(x))/len(s) for x in range(97,120))]
...
def decode(encmessage, key=englishPercents):
    #re-encode it for every number
    distances = [dist(calcPercents(encode2(encmessage,x)), key) for x in range(26)]
    return encode2(message, distances.index(min(distances)))

def calcPercent(f):
    text = open(f)
    i= calcPercents(text.read())
    text.close()
    print i

decode(encmessage, englishPercents)
decode(encmessage, calcPercent('corpus.txt'))
