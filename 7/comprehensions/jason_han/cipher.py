...
def dist2(a,b):
    x = [pow(a[i]-b[i],2) for i in range(len(a))]
    y = sum(x)
    z = math.sqrt(y)
    return z

#print dist([1,2,3],[4,5,6])
#print dist2([1,2,3],[4,5,6])

def calcPercents(s):
    x = len(s) - s.count(" ")
    y = [100 * float(s.count(chr(a + 97)))/x for a in range(26)]
    return y
    """
...
def bestCalc(s,t):
    x = [calcPercents(encode2(s,a)) for a in range(26)]
    y = [dist(b,t) for b in x]
...
t = encode2(s,1)
print t
print bestCalc(t,englishPercents)
...
bookPercents = calcPercents(data)
u = encode2(s,1)
print bestCalc(u,bookPercents)
